...
class SaasFrontController extends SaasFrontBaseController
{
    ...
    {
        if ($this->global->setup_homepage == 1) {
        ...
        $this->featuredCompanies = Company::where('companies.status', 'active')
        ->join('company_packages', 'company_packages.id', 'companies.package_id')
        ->join('packages', 'company_packages.package_id', 'packages.id')
            ...
            ->where(function ($query) use($currentDate) {
                ...
                ->where(function ($query) use($currentDate) {
                    $query->where(DB::raw('DATE(company_packages.end_date)'), '>=', DB::raw('CURDATE()'))
                    ->orWhereNull('company_packages.end_date');
                })

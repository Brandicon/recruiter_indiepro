<a href="javascript:;" data-link="{{ route('admin.job-applications.index') }}" class="dropdown-item text-sm read-notification" data-notification-id="{{ $notification->id }}">
    ...
    <span class="text-truncate-notify" style="overflow-y: hidden" title="full name">{{ __('messages.notifications.candidateScheduleInterview'), ['name' => $notification->data['data']['full_name']]) }}</span>

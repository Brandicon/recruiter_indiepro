@php
    $you = ' ';
    if ($notification->data['data']['id'] == $user->id) {
        $you = ' ( '.__('app.you').' )';
    }
@endphp
...
<a href="javascript:;" data-link="{{ route('admin.job-applications.index') }}" class="dropdown-item text-sm read-notification" data-notification-id="{{ $notification->id }}">
    ...
    <span class="text-truncate-notify" style="overflow-y: hidden" title="full name">{{ __('messages.notifications.employeeResponse', ['name' => $notification->data['data']['name'].$you]) }}</span>

                ...
                <div class="col-md-12">
                    @if(!is_null($packageData) && $paymentSetting->stripe_status == 'active')
                        ...
                        <div class="alert alert-danger col-md-12">
                            <div class="col-md-12"> @lang('messages.PlanIdRequired')</div>
                        </div>
                    @endif
                </div>
                    ...
                    <tbody>
                        ...
                        <tr id="row-{{ $item->id }}">
                            ...
                            <td>{{ $global->currency->currency_symbol.ucfirst($item->monthly_price) }}
                                @if($paymentSetting->stripe_status == 'active' && $item->is_trial != 1)
                                    <br>(@lang('app.stripePlanId') : {{ $item->stripe_monthly_plan_id ?? '-' }})<br>
                                    (@lang('app.razorPayId') : {{ $item->razorpay_monthly_plan_id ?? '-' }})
                            ...
                            <td>{{ $global->currency->currency_symbol.ucfirst($item->annual_price) }}
                                @if($paymentSetting->stripe_status == 'active' && $item->is_trial != 1)
                                    <br>(@lang('app.stripePlanId') : {{ $item->stripe_annual_plan_id ?? '-' }})<br>
                                        (@lang('app.razorPayId') : {{ $item->razorpay_annual_plan_id ?? '-' }})
                            ...
                            <td>
                                <ul class="list-unstyled">
                                    ...
                                    <li class="mt-2">
                                        <small>
                                        @lang('modules.packages.noOfJobOpenings'): {!! ($item->no_of_job_openings > 0) ? $item->no_of_job_openings : __('app.unlimited') !!}
                                        </small>
                                    </li>
                                    <li class="mt-2">
                                        <small>
                                        @lang('modules.packages.noOfCandidateAccess'): {!! ($item->no_of_candidate_access > 0) ? $item->no_of_candidate_access : __('app.unlimited') !!}
                                        </small>
                                    </li>
